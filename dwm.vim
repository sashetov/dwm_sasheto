...
set splitbelow
set splitright
set statusline=%{ChangeStatuslineColor()}%0*\ %{toupper(g:currentmode[mode()])}%8*\ [%n]%8*\ %{GitInfo()}%8*\ %<%F\ %{ReadOnly()}\ %m\ %w\\%#warningmsg#%{SyntasticStatuslineFlag()}%*%9*\ %=%8*\ %y\\%7*\ %{(&fenc!=''?&fenc:&enc)}[%{&ff}]\\%8*\ %-3(%{FileSize()}%)%0*\ %3p%%\ \ %l:\ %3c\\
set tabpagemax=100
set tabstop=2
...
silent only
cd /srv/data/src/dwm-user-6.1-4.fc26
...
badd +6 rpmbuild.sh
badd +1 makerpm
badd +1 rpmbuild_clean.sh
badd +1 rpminstall.sh
badd +1 config.def.h
badd +1 dwm.c
badd +1 dwm.h
badd +1 drw.c
...
let s:cpo_save=&cpo
set cpo&vim
map <buffer> \c <Plug>SpecChangelog
let &cpo=s:cpo_save
unlet s:cpo_save
setlocal keymap=
setlocal noarabic
...
wincmd w
argglobal
if bufexists('rpmbuild.inc.sh') | buffer rpmbuild.inc.sh | else | edit rpmbuild.inc.sh | endif
setlocal keymap=
setlocal noarabic
setlocal noautoindent
setlocal backupcopy=
...
wincmd w
argglobal
if bufexists('rpminstall.sh') | buffer rpminstall.sh | else | edit rpminstall.sh | endif
setlocal keymap=
setlocal noarabic
setlocal noautoindent
setlocal backupcopy=
...
wincmd w
argglobal
if bufexists('rpmbuild_clean.sh') | buffer rpmbuild_clean.sh | else | edit rpmbuild_clean.sh | endif
setlocal keymap=
setlocal noarabic
setlocal noautoindent
setlocal backupcopy=
...
wincmd w
argglobal
if bufexists('config.mk') | buffer config.mk | else | edit config.mk | endif
setlocal keymap=
setlocal noarabic
setlocal noautoindent
setlocal backupcopy=
...
wincmd w
argglobal
if bufexists('drw.h') | buffer drw.h | else | edit drw.h | endif
nnoremap <buffer> <silent> - :CCTreeRecurseDepthMinus
nnoremap <buffer> <silent> = :CCTreeRecurseDepthPlus
setlocal keymap=
setlocal noarabic
